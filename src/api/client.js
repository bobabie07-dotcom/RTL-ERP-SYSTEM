...
async function request(method, path, body) {
  ...
  const timer = setTimeout(() => controller.abort(), 60000); // 60s — covers Render cold start (~30-45s)
  ...
  try {
    ...
    clearTimeout(timer);
    if (err.name === 'AbortError') throw new Error('Server is still waking up. Please wait a moment and try again.');
    throw err;
  }
  clearTimeout(timer);
...
export const feedApi = {
  ...
  standardSchedule:()             => get('/api/feed/standard-schedule'),
  createSchedule:  (data)         => post('/api/feed/standard-schedule', data),
  updateSchedule:  (id, data)     => patch(`/api/feed/standard-schedule/${id}`, data),
  deleteSchedule:  (id)           => del(`/api/feed/standard-schedule/${id}`),
  batchStandard:   (batchId, params) => get(`/api/feed/batches/${batchId}/standard`, params),
  batchTimeline:   (batchId, params) => get(`/api/feed/batches/${batchId}/timeline`, params),
  standardVariance:(params)       => get('/api/feed/standard-variance', params),
  standardReport:  (params)       => get('/api/feed/standard-report', params),
...
export const salesApi = {
  ...
  updateExpense: (id, data)  => patch(`/api/sales/expenses/${id}`, data),
  deleteExpense: (id)        => del(`/api/sales/expenses/${id}`),
