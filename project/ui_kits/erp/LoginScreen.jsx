...
const { Button, Input, Checkbox, IconButton } = window.RTLPoultryFarmingERPDesignSystem_698a73;
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div style={{ display: 'flex', height: '100%', minHeight: 560, fontFamily: 'var(--font-body)' }}>
      {/* Hero */}
      <div style={{
        flex: '1 1 52%',
        position: 'relative',
        ...
        <div style={{ position: 'absolute', inset: 0, backgroundImage: `url(${(window.__resources && window.__resources.heroPoultry) || '../../assets/hero-poultry.png'})`, backgroundSize: 'cover', backgroundPosition: 'center', opacity: 0.22 }} />
        <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(135deg, rgba(7,31,17,0.86) 0%, rgba(7,31,17,0.62) 100%)' }} />
        <div style={{ position: 'relative' }}>
          <img src={(window.__resources && window.__resources.logoLockup) || '../../assets/logo-lockup-dark.png'} alt="RTL Poultry Farming ERP" style={{ height: 56 }} />
        </div>
        <div style={{ position: 'relative' }}>
      ...
      <div style={{ flex: '1 1 48%', background: 'var(--surface-card)', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 40 }}>
        <form
          onSubmit={(e) => { e.preventDefault(); onLogin && onLogin(); }}
        ...
        >
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, marginBottom: 6 }}>
            <span style={{ width: 64, height: 64, borderRadius: '50%', background: 'var(--green-50)', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>
              <img src={(window.__resources && window.__resources.logoMark) || '../../assets/logo-mark.png'} alt="" style={{ width: 46, height: 46, objectFit: 'contain' }} />
            </span>
