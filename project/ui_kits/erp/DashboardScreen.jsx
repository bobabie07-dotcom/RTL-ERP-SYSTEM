...
const { StatCard, Card, DataTable, Badge, Button, Select } = window.RTLPoultryFarmingERPDesignSystem_698a73;

function DashboardScreen({ onOpenBatch }) {
  const I = window.RTLIcons;
  const { LineChart, DonutChart, BarChart } = window.RTLCharts;
  ...
  return (
    <div style={{ padding: 24, display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <Card title="Active Batches" action={<Button variant="secondary" size="sm" icon={<I.chevronRight w={15} />}>View All Batches</Button>}>
        <DataTable
          columns={[
            { key: 'batch', header: 'Batch No.', strong: true, render: (r) => (
              <a href="#" onClick={(e) => { e.preventDefault(); onOpenBatch && onOpenBatch(r); }} style={{ fontWeight: 600 }}>{r.batch}</a>
            ) },
